...
Surya Summary: I organized the project data into appropriate folders, with data_final containing the final datasets used for analysis. We decided to use only the UK subset from the WHO dataset so it better matched the project scope, and I converted the original .ods air quality file into .csv to make it easier to work with in Python. I also hashed the datasets for integrity checking and loaded them using relative paths, so the code fits the repository structure and can run properly on any machine when someone clones the GitHub repo. I also identified the data integration methods we will likely use later, although that part has not been implemented yet.
